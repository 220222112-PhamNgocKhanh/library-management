...
public class FileHandler {
    ...
    public void note() {
        ...
        if (noteFile.exists()) {
            try (BufferedReader reader = new BufferedReader(new FileReader(noteFile, StandardCharsets.UTF_8))) {
                StringBuilder content = new StringBuilder();
                String line;
                while ((line = reader.readLine()) != null) {
                    content.append(line).append("\n");
        ...
        noteStage.setOnCloseRequest(event -> {
            try (BufferedWriter writer = new BufferedWriter(new FileWriter(noteFile, StandardCharsets.UTF_8))) {
