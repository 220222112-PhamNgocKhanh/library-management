...
public class Interface  {
  ...
  private VBox createSidebar() {
    ...
    for (Button btn : new Button[]{homeButton, bookButton, borrowerButton, noteButton,
        helpButton}) {
      btn.setStyle(
      ...
      btn.setOnAction(e -> {
        ...
        // Gọi phương thức tương ứng
          if (btn == homeButton) {
              ...
              new FileHandler().help();
          } else if (btn == bookButton) {
              switchToBookManagement();
          } else if (btn == noteButton) {
              new FileHandler().note();
          } else if (btn == borrowerButton) {
              switchToBorrowerManagement();
          }
      });
    }
    ...
    sidebar.getChildren()
        .addAll(title, homeButton, bookButton, borrowerButton, noteButton, helpButton);
    ...
    ImageView memberIcon = new ImageView(
        new Image(getClass().getResourceAsStream("/icon/member.png")));
    ...
    ImageView logoutIcon = new ImageView(
        new Image(getClass().getResourceAsStream("/icon/logout.png")));
  ...
  private void highlightButton(Button button) {
    ...
    if (selectedButton != null) {
      selectedButton.setStyle(
          "-fx-background-color: transparent; -fx-text-fill: white; -fx-font-size: 16px; -fx-font-weight: bold; -fx-alignment: CENTER_LEFT;");
    ...
    button.setStyle(
        "-fx-background-color: #FFFFFF; -fx-text-fill: #FF7E5F; -fx-font-size: 16px; -fx-font-weight: bold; -fx-alignment: CENTER_LEFT;");
  ...
  private VBox createMainContent() {
    ...
    titleLabel.setFont(
        Font.font("Arial", FontWeight.BOLD, 36)); // Tăng kích thước chữ và làm chữ đậm
    ...
    Label box1Content = new Label(
        "Chào mừng bạn đến với Library Management System, một ứng dụng đơn giản giúp "
    ...
    // Hình ảnh bên trái
    ImageView featureImage2 = new ImageView(
        new Image("laptop.jpg")); // Đường dẫn ảnh từ file tải lên
    featureImage2.setFitWidth(180); // Kích thước hình ảnh
    featureImage2.setPreserveRatio(true);
    featureImage2.setStyle(
        "-fx-border-color: #ccc; -fx-border-width: 2px; -fx-border-radius: 15px;");
    ...
    Label featureLabel2 = new Label(
        "- Quản lý sách: Thêm, xóa, và cập nhật thông tin sách trong thư viện.\n"
